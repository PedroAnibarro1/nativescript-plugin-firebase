      ...
      const onCompleteListener = new gmsTasks.OnCompleteListener({
        onComplete: task => {
          if (!task.isSuccessful()) {
            ...
            if (task.getException() && task.getException().getErrorCode() === "ERROR_CREDENTIAL_ALREADY_IN_USE") {
              console.log("It's a phone collision!!!!!");
              const newCredentials = task.getException().getUpdatedCredential();
              firebaseAuth.signInWithCredential(newCredentials).addOnCompleteListener(onCompleteListener);
            } else {
              // also disconnect from Google otherwise ppl can't connect with a different account
